const filmy = [
	...
	{
		id: "avengers",
		nazev: "Avengers: The Kang Dynasty",
		plakat: {
			url: "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/1b271d0d-1fcc-4802-bc62-1ac6000a0e07/dfaewg8-522d79c7-6cdb-4114-ae0e-26c5f70e81e0.jpg/v1/fill/w_1280,h_1600,q_75,strp/avengers_the_kang_dynasty___marvel_studios_by_pablomyths_dfaewg8-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..74xGoQcalfIwwSeVluz83SlR99AS2zjugh9pazVNT2I",
			sirka: 420,
			vyska: 592,
		},
		ochutnavka: "Akční, dobrodružný, Sci-Fi, fantasy",
		popis: "Marvel Studios Avengers: The Kang Dynasty will arrive in theaters on May 1, 2026",
		premiera: "2026-05-01"
	}
]

const seznam = document.querySelector("#detail-filmu")
...
seznam.querySelector(".card-title").textContent = film.nazev
seznam.querySelector(".card-text").textContent = film.popis

const plakat = seznam.querySelector(".img-fluid")
plakat.src = film.plakat.url
plakat.width = film.plakat.sirka
plakat.height = film.plakat.vyska

const datum = seznam.querySelector("#premiera")
const premiera = dayjs(film.premiera)
const premieraFormat = premiera.format('D. M. YYYY')
const dnes = dayjs()
const dny = Math.abs(premiera.diff(dnes, "days"))

if(dnes.isAfter(premiera)) {
	datum.innerHTML = `Premiéra <strong>${premieraFormat}</strong>, což bylo před ${dny} dny.`
} else {
	datum.innerHTML = `Premiéra <strong>${premieraFormat}</strong>, což bude za ${dny} dní.`
} 

const hodnoceni = document.querySelector(".stars")


const hodnoceniHvezdicky = (pocet) => {
	const hvezdicky = hodnoceni.querySelectorAll(".fa-star")
...
const posledniHvezdicka = hodnoceni.querySelectorAll(".fa-star")


posledniHvezdicka.forEach((hvezdicka, index) => {
	hvezdicka.addEventListener("click", () => {
		posledni = index + 1
		hodnoceniHvezdicky(posledni)
	})

	hvezdicka.addEventListener("mouseenter", () => {
		hodnoceniHvezdicky(index + 1)
	})
